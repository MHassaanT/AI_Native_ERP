            ...
            if node.agent_id == "REVENUE":
                ...
                # 4. Auto-Provision Customer & Confirmed Sales Order
                    ...
                    if not item_found:
                        ...
                        async with async_session_factory() as s:
                            ...
                            # Lookup item from DB to ensure real catalog rate is used
                            ...
                            if is_in_stock:
                                ...
                                if wh_id:
                                    stk_stmt = stk_stmt.where(StockLevel.warehouse_id == wh_id)
                                stk_rows = (await s.execute(stk_stmt)).all()
                                if stk_rows:
                                    stk_row = next((r for r in stk_rows if r[0].available_qty >= qty), stk_rows[0])
                                    stk, wh_obj = stk_row[0], stk_row[1]
                                    wh_id = wh_obj.warehouse_id
                ...
                # 5. Outbound Order Confirmation & Invoice Dispatch
                    ...
                    if not item_found:
                        ...
                        # Generate official PDF invoice bytes with real catalog item
